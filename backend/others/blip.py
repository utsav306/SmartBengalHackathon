...
def get_clip_prompt(section_type, category):
    prompts = {
        "header": f"Evaluate the {section_type} section of a {category} website based on usability, branding consistency, and navigation clarity.",
        "main": f"Evaluate the {section_type} section of a {category} website based on content clarity, visual hierarchy, and user engagement.",
        "footer": f"Evaluate the {section_type} section of a {category} website based on information completeness, accessibility, and contact options."
    }
...
# --- Phase 2: Score each section using BLIP ---
@torch.no_grad()
def score_section(image_path, section_type, category):
    ...
    image_embed = outputs.vision_model_output.last_hidden_state[:, 0, :]
    text_embed = outputs.text_model_output.last_hidden_state[:, 0, :]

    image_embed = image_embed / image_embed.norm(dim=-1, keepdim=True)
    text_embed = text_embed / text_embed.norm(dim=-1, keepdim=True)

    similarity = cosine_similarity(image_embed, text_embed).item()
    normalized_similarity = (similarity + 1) / 2  # range 0 to 1

    # Adjusted scores based on section-specific criteria weights
    ...
    print(f"📷 {section_type.capitalize()} - {os.path.basename(image_path)}: BLIP Score = {normalized_similarity:.3f}")
